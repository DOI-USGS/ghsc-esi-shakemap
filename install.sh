...
usage()
{
  echo "Usage: install.sh [ -d  Developer]
            "
  exit 2
}
...
if [ ! -d "$matplotlibdir" ]; then
    if [[ $(mkdir -p $matplotlibdir) != 0 ]];then
...
if ! conda --version ; then
    ...
    if ! curl -L $mini_conda_url -o miniconda.sh ;then
    ...
    if ! bash miniconda.sh -f -b -p "$HOME/miniconda" ;then
...
if ! ${install_pgm} env create -f ${input_yaml_file} ; then
...
if $developer; then
    ...
    if ! pip install '.[dev,test,doc]' ; then
